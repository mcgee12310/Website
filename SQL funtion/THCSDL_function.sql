...
RETURNS void AS $$
BEGIN
	ALTER TABLE "Lịch sử ra vào" drop CONSTRAINT "Biển số";
...
Cách 2:Có thể bỏ 2 câu lệch CONSTRAINT nếu bỏ khóa ngoài tham chiếu đến bảng “Lịch sử ra vào”. => Giảm thời gian thực hiện.
CREATE OR REPLACE FUNCTION delete_xe(bien_so text)
RETURNS void AS $$
BEGIN
		DELETE FROM "Xe" WHERE "Biển số" = bien_so;
...
RETURNS void AS $$
BEGIN
	ALTER TABLE "Hóa đơn" drop CONSTRAINT "ID Khách";
...
END;
$$ LANGUAGE plpgsql;
Cách 2: Có thể bỏ 2 câu lệch CONSTRAINT nếu bỏ khóa ngoài tham chiếu đến bảng “Hóa đơn”. => Giảm thời gian thực hiện.
CREATE OR REPLACE FUNCTION delete_khach(ten text, sdt text)
RETURNS void AS $$
BEGIN
    delete from "Khách" where "Họ tên" = ten and "SĐT" = sdt;
...
Cách 2: Viết từng truy vấn cập nhập trong từng trường thông tin. Ví dụ:
update "Cư dân" set "Họ tên"=new_ho_ten where "Họ tên"=old_ho_ten and "Căn hộ"=room;
=> Nếu muốn update nhiều hơn 1 trường thông tin thì sẽ phải viết truy vấn nhiều lần 
=> cách 1 sẽ tối ưu hơn
...
Cách 2: Viết từng truy vấn cập nhập trong từng trường thông tin. Ví dụ:
update "Xe" set "Biển số"=new_bien_xe where "Biển số"=old_bien_xe;end if;
=> Nếu muốn update nhiều hơn 1 trường thông tin thì sẽ phải viết truy vấn nhiều lần 
=> cách 1 sẽ tối ưu hơn
...
Cách 2:
update "Phí dịch vụ" set "Phí vệ sinh"=phi_vs where where "Căn hộ"=can_ho
	and thang = (select EXTRACT(month FROM "Thời gian")) 
	and (select EXTRACT(year FROM "Thời gian"))=(select EXTRACT(year FROM now()));
=> Nếu muốn update nhiều hơn 1 trường thông tin thì sẽ phải viết truy vấn nhiều lần 
=> cách 1 sẽ tối ưu hơn
...
Cách 2:
update "Khách" set "Họ tên"=new_ho_ten where "Họ tên"=old_ho_ten and "SĐT"=old_sdt;
=> Nếu muốn update nhiều hơn 1 trường thông tin thì sẽ phải viết truy vấn nhiều lần 
=> cách 1 sẽ tối ưu hơn

Tìm thông tin của admin:
Cách 1:
CREATE OR REPLACE FUNCTION info_admin1()
...
Cách 2:
select  * from "Cư dân" join "Xe" on "Cư dân"."Căn hộ"="Xe"."Căn hộ" 
where "Cư dân"."Căn hộ"=can_ho;
=> Truy vấn ở cách 2 sẽ trả về nhiều giá trị trùng nhau nên cách 1 sẽ tối ưu hơn.
...
Có thể gọi hàm sau để tự động thêm phí dịch vụ cho tất cả căn hộ nhưng vẫn phải update lại số điện, nước:
CREATE OR REPLACE FUNCTION process_all_phi_dv()
RETURNS void AS $$
DECLARE
    can_ho text;
    can_ho_cursor CURSOR FOR
        SELECT "Căn hộ"
        FROM "Căn hộ";
BEGIN
    OPEN can_ho_cursor;
    LOOP
        FETCH can_ho_cursor INTO can_ho;
        EXIT WHEN NOT FOUND;
        -- Gọi hàm xử lý từng căn hộ
        PERFORM phi_dv(can_ho, 1, 1);
    END LOOP;
    CLOSE can_ho_cursor;
END;
$$ LANGUAGE plpgsql;
=> Query complete 00:00:04.404 ~ 00:00:04.919
...
RETURNS void AS $$
BEGIN  
	select "Căn hộ" into can_ho from "Phí dịch vụ" where "Căn hộ"=can_ho
	and thang = (select EXTRACT(month FROM "Thời gian")) 
	and "Trạng thái"='Chưa thanh toán';
	
	if can_ho is null 
then 
raise notice 'Thanh toán không thành công!'; 
return; 
end if;
...
END;
$$ LANGUAGE plpgsql;
=> Query complete 00:00:00.077 ~ 00:00:00.163 
Cách 2:
CREATE OR REPLACE FUNCTION thanh_toan_phi_dv(can_ho text, thang integer)
RETURNS void AS $$
declare key int;
...
END;
$$ LANGUAGE plpgsql;
=> Query complete 00:00:00.062 ~ 00:00:00.172  
=> Cách 1 truy vấn UPDATE sẽ mất thời gian hơn cách 2 do cách 2 tìm bản ghi theo duy nhất khóa chính => thực hiện nhanh hơn.
